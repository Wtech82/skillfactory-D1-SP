...
if os.path.isfile("user_json.txt"):
    print ("Файл авторизации существует")
    
    file_object = open("user_json.txt")
    ...
    print ("Файл авторизации не существует. Сейчас запишем данные.")
    base_url = "https://api.trello.com/1/{}"
    key = input("Введи key Trello: ")
    token = input("Введи token Trello: ")
    board = input("Введи короткий board_id доски Trello: ")
    auth_params = {    
        'key': key,    
        'token': token, }
    board_id = board
    ...
    file_object = open("user_json.txt", "w")
    # сохраняем словарь user в объект файла file_object
    json.dump(user, file_object)
    # закрываем объект файла
    file_object.close()

    print('Файл авторизации user_json.txt создан. Авторизация в дальнейшем не потребуется!')
...
def read():      
    ...
    print('Список доступных функций: create(name, column_name) - создать; move(name, column_name) - переместить')
    print('Для работы используйте терминал (python trello.py create "New task" "To Do")')
